...
def affine_backward(dout, cache):
    """计算 反向传播 用于 an affine (fully connected) 层.
...
def dropout_forward(x, dropout_param):
    """前向传播 用于 inverted dropout.

    Note 该 这个 is different 来自 vanilla version 的 dropout.
    Here, p is probability 的 keeping a neuron 输出, as opposed to
    probability 的 dropping a neuron 输出.
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    In contrast 到 层 归一化, group 归一化 splits each entry 在 数据 到 G
    contiguous pieces, which it 然后 normalizes independently. Per-特征 平移 并 scaling
    are 然后 applied 到 数据, 在 a manner identical 到 该 的 batch 归一化 并 层
    归一化.
